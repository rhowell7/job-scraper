...
def in_usa(location):
    # List of keywords to include US-based jobs
    allow_keywords = [
        'remote', 'united states', 'us', 'americas', 'silicon valley'
    ]
    
    # List of US states and major cities
    states_and_cities = [
        ...
        'farmington', 'fl', 'florida', 'ga', 'georgia', 'houston', 'ia', 'il',
        'illinois', 'in', 'indiana', 'indianapolis', 'iowa', 'kansas', 'kentucky',
        'ks', 'ky', 'la', 'las vegas', 'los angeles', 'louisiana', 'ma', 'madison',
        'maine', 'maryland', 'massachusetts', 'md', 'me', 'mi', 'michigan',
        'milwaukee', 'minneapolis', 'minnesota', 'mississippi', 'missouri', 'mn',
        'mo', 'montana', 'ms', 'mt', 'nashville', 'nc', 'ne', 'nebraska',
        'nevada', 'new hampshire', 'new jersey', 'new mexico', 'new york', 'nh',
        'nj', 'nm', 'north carolina', 'nv', 'ny', 'nyc', 'oh', 'ohio', 'ok',
        'oklahoma', 'or', 'oregon', 'pa', 'pennsylvania', 'philadelphia', 'phoenix',
        'pittsburgh', 'portland', 'raleigh', 'rhode island', 'ri', 'royal oak',
        'saint paul', 'salt lake city', 'san diego', 'san francisco', 'sc', 'seattle',
        'south carolina', 'southfield', 'tennessee', 'texas', 'tn', 'troy', 'tx',
        'ut', 'utah', 'va', 'vermont', 'virginia', 'vt', 'wa', 'washington',
        'west virginia', 'wi', 'wisconsin', 'wv', 'wy', 'wyoming']
    
    # Keywords to exclude non-US jobs
    exclude_keywords = [
        'canada', 'latam', 'europe', 'mexico', 'germany', 'uk', 'apac',
        'london'
    ]
    
    # Normalize the location string to lowercase
    location = location.lower()
    
    # Split the location into parts using delimiters
    ...
    # If no match is found, default to including the job
    return True
...
def clean_text(text):
    ...
    text = "\n".join(line.strip() for line in text.splitlines())  # Trim leading/trailing spaces
    return text
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'greenhouse' in url:
                # Greenhouse pattern: "Job Application for [Job Title] at [Company Name]"
                ...
                pattern = re.compile(r'"job_post_location":"(.*?)","public_url":"(.*?)","company_name":"(.*?)"')
                ...
                if match:
                    ...
                    job_description_div = soup.find('div', class_='job__description body')
            ...
            # Lever job board
                ...
                job_description_tag = soup.find('div', attrs={'data-qa': 'job-description'})
...
def extract_salary(description):
    ...
    if len(matches) >= 2:
        ...
        # If only one salary is found, return it as the minimum and leave max_salary as None
        min_salary = int(matches[0].replace(',', ''))
...
def get_glassdoor_data_from_db(company_name):
    ...
    """
    cursor.execute("SELECT glassdoor_rating, glassdoor_link, glassdoor_num_reviews, company_size FROM glassdoor_data WHERE company = %s", (job_info['company_name'],))
...
def save_glassdoor_data_to_db(company_name, glassdoor_data):
    ...
    """
    cursor.execute(
        "INSERT INTO glassdoor_data (company, glassdoor_rating, glassdoor_link, glassdoor_num_reviews, company_size) VALUES (%s, %s, %s, %s, %s)",
        (company_name, glassdoor_data['rating'], glassdoor_data['glassdoor_url'], glassdoor_data['reviews'], glassdoor_data['company_size'])
    )
    conn.commit()
...
def scrape_glassdoor_data(company_name):
    ...
    try:
        ...
        try:
            ...
            try:
                rating = first_company_tile.find_element(By.CSS_SELECTOR, 'strong.small.css-b63kyi').text.strip().replace(' ★', '')
            except Exception:
                rating = 'N/A'

            # Extract the number of reviews
            try:
                reviews_span = first_company_tile.find_elements(By.XPATH, ".//span[contains(text(),'Reviews')]")[0]
                reviews = reviews_span.find_element(By.XPATH, "./preceding-sibling::span").text.strip()
            except Exception:
                reviews = 'N/A'

            # Extract the company size
            try:
                company_size = first_company_tile.find_element(By.XPATH, ".//span[contains(text(),'Employees')]").text.strip()
            except Exception:
                company_size = 'N/A'
...
if __name__ == "__main__":
    ...
    query = (
        '(intitle:engineer OR intitle:software OR intitle:python OR intitle:developer OR intitle:data) '
        '(python OR backend OR ml OR "machine learning") '
        'site:lever.co OR site:greenhouse.io '
        '-intitle:staff -intitle:senior -intitle:manager -intitle:lead -intitle:principal '
        '-intitle:director "Remote"'
    )
    
    results = google_custom_search(query, 8)
    ...
    if results:
        for item in results.get('items', []):
            ...
            if not job_info or not job_info['job_title'] or not job_info['company_name']:
                print(f"  378: Failed to extract job and company from {url}")
                continue

            # Skip jobs outside the USA
            if not in_usa(job_info['location']):
                print(f"  383: Skipping job outside the USA:")
                print(f"    Location: {job_info['location']}")
                print(f"    URL: {url}")
                print(f"    Title: {job_info['job_title']}")
                print(f"    Company: {job_info['company_name']}")
                cursor.execute(
                    "INSERT INTO foreign_jobs (url, company, job_title, location, date_first_seen) VALUES (%s, %s, %s, %s, CURRENT_DATE)",
                    (url, job_info['company_name'], job_info['job_title'], job_info['location'])
                )
                conn.commit()
                continue


            # if job_info['location'] and job_info['description']:
            score, preference_hits = score_job(job_info['job_title'], job_info['description'])
            ...
            glassdoor_data = get_glassdoor_data_from_db(job_info['company_name'])

            if glassdoor_data is None:
                print(f"  405: Scraping Glassdoor data for {job_info['company_name']}...")
                glassdoor_data = scrape_glassdoor_data(job_info['company_name'])
                save_glassdoor_data_to_db(job_info['company_name'], glassdoor_data)
            else:
                print(f"  409: Using cached Glassdoor data for {job_info['company_name']}")

            #  id | company | job_title | date_first_seen | salary_min | salary_max | location | url | glassdoor_rating | glassdoor_link | glassdoor_num_reviews | company_size | score | keywords | preference_hits
            cursor.execute(
                "INSERT INTO jobs (company, job_title, date_first_seen, salary_min, salary_max, location, url, glassdoor_rating, glassdoor_link, glassdoor_num_reviews, company_size, score, preference_hits, keywords) VALUES (%s, %s, CURRENT_DATE, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (job_info['company_name'], job_info['job_title'], salary_min, salary_max, job_info['location'], url, glassdoor_data['rating'], glassdoor_data['glassdoor_url'], glassdoor_data['reviews'], glassdoor_data['company_size'], score, preference_hits, keywords)
            )                
            conn.commit()
